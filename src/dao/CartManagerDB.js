...
const usertModelCart = mongoose.model(
...
export default class CartManagerDB {
  ...
  getCarts = async (limit = 10, page = 1, query = "{}", sort) => {
    ...
    const carts = await this.cartModel.paginate(vquery, {
      page,
      limit,
      lean: true,
      sort,
  ...
  addCart = async (uid) => {
    ...
    try {
      ...
      await newCart.save();
      const founduser = await usertModelCart
        .findOne({ _id: uid })
        .lean()
        .exec();
      if (founduser === null) {
      ...
      const updatedUser = await usertModelCart.findByIdAndUpdate(uid, {
        cart: cid,
      });
...
`,
      ...
      return {
        status: outofstock_id.length === 0 ? "Completed" : "Out of stock",
